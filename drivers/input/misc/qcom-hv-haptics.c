		...
		struct haptics_chip *chip, bool in_boot)
{
	...
	if (in_boot) {
		...
		rc3 = of_property_read_u32(node, "qcom,lra-f0-default",
					   &f0_default);
		if (rc3 < 0) {
			dev_err(chip->dev, "lra-f0-default failed, rc=%d\n",
				rc);
		}
		rc4 = of_property_read_u32(node, "qcom,lra-f0-cal-count",
					   &f0_cnt);
		if (rc4 < 0) {
		...
		if (rc1 >= 0 && rc2 >= 0 && rc3 >= 0 && rc4 >= 0) {
			if (xbl_f0 > f0_max || xbl_f0 < f0_mix) {
				dev_info(
					chip->dev,
					"xbl f0 abnormal: %d ~ 0x%x use default: %d ~ 0x%x f0:%d - %d after boot\n",
					xbl_f0, config->rc_clk_cal_count,
					f0_default, f0_cnt, f0_mix, f0_max);
				XM_HAP_F0_CAL_EXCEPTION(
					xbl_f0, f0_default,
					"F0 Calibration failed, use default val");
				config->cl_t_lra_us = USEC_PER_SEC / f0_default;
				config->rc_clk_cal_count = f0_cnt;
			}
		} else {
			dev_err(chip->dev,
				"lra-f0: default min max count must set together in dtsi\n");
			XM_HAP_F0_PROTECT_EXCEPTION(
				xbl_f0, "haptics_get_closeloop_lra_period");
		}
	}
#endif
...
#ifdef CONFIG_TARGET_PRODUCT_GARNET
	...
	if (!!val) {
		...
		if (play->pattern_src == DIRECT_PLAY) {
			...
			hrtimer_start(&chip->dph_off_timer,
				      ns_to_ktime(length_ns), HRTIMER_MODE_REL);
		}
	} else {
		if (play->pattern_src == FIFO &&
...
static int haptics_detect_lra_frequency(struct haptics_chip *chip)
{
	...
	rc = haptics_masked_write(
		chip, chip->cfg_addr_base, HAP_CFG_AUTORES_CFG_REG,
		AUTORES_EN_BIT | AUTORES_EN_DLY_MASK | AUTORES_ERR_WINDOW_MASK,
...
static enum hrtimer_restart haptics_disable_dph_timer(struct hrtimer *timer)
{
	struct haptics_chip *chip =
		container_of(timer, struct haptics_chip, dph_off_timer);
	int rc;
	rc = haptics_enable_play(chip, false);
	if (rc < 0)
